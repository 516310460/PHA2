export default (context, inject) => {
    ...
    im.orderNotice = {
        // 订单变动推送功能
        RECIVE_ORDER_NOTICE: "RECIVE_ORDER_NOTICE",
    };
        ...
        if (!!param) {
            if (!!param.namespace) {
                ...
                if (im.namespace.LOGIN_REGIST === param.namespace) {
                    if (!!param.token) {
                        paramTxt =
                            "?token=" + param.token + "&namespace=" + im.namespace.LOGIN_REGIST;
                    } else {
                    ...
                    if (!!param.activityId) {
                        paramTxt = paramTxt + "&activityId=" + param.activityId;
                    }
                    if (!!guestRegistFunction) {
                        ...
                        if (
                            ...
                            guestRegistFunction === im.guestRegistFunction.ACTIVITIES_MSG ||
                            guestRegistFunction === im.guestRegistFunction.CALCULATOR_MSG
                        ) {
    ...
    im.heartBit = {
        ...
        start: () => {
            ...
            if (!!im.heartBit.connect_timer) {
                clearTimeout(im.heartBit.connect_timer);
            }
            im.heartBit.heart_timer = setTimeout(() => {
                ...
                if (!ImRouteState) {
                    ...
                    // 休眠了执行
                    if (context.route.path == `/${context.params.lang}/` ||
                        context.route.path == `/${context.params.lang}/Activity` ||
                        context.route.path == `/${context.params.lang}/Activity/OrderDetails` ||
                        context.route.path == `/${context.params.lang}/AggregatedTransaction/OrderDetails` ||
                        context.route.path == `/${context.params.lang}/CoinCity/OrderDetails` ||
                        context.route.path == `/${context.params.lang}/OTC/amendAd` ||
                        context.route.path == `/${context.params.lang}/OTC/OrderDetails` ||
                        context.route.path == `/${context.params.lang}/OTC/USDTOrderDetails` ||
                        context.route.path == `/${context.params.lang}/OTC/IssueAdvertise` ||
                        context.route.path == `/${context.params.lang}/OTC/IssueAdvertise`) {
                        im.websocket.send("{'ping':" + new Date().getTime() + "}");
                        im.heartBit.connect_timer = setInterval(() => {
                            if (im.websocket !== null) {
                                im.websocket.close();
                                if (!!im.initparam) {
                                    im.init(im.initparam);
