...
export default (context, inject) => {
  ...
  let plocalStorage = {
    crossDomainStorage(opts) {
      ...
      // cdstorage.getItem = function(key) {
      //   if (supported) {
      //     var request = {
      //         id: ++_id,
      //         type: 'get',
  ...
  var options = {
    origin: "http://www.devbuybit.com",
    path: "/html/crossd_iframe.html"
  };

  if (process.client) {
    if (window.location.href.includes(process.env.PATH_API[context.store.state.env_name].hashRateUrl)) {
      options.origin = process.env.PATH_API[context.store.state.env_name].otcUrl
    } else {
      options.origin = process.env.PATH_API[context.store.state.env_name].hashRateUrl
    }
  }

  context.$crossSession = plocalStorage.crossDomainStorage(options);
  inject("crossSession", plocalStorage.crossDomainStorage(options));
};
