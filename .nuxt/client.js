...
const NUXT = window.__COINPOOL__ || {}
...
async function loadAsyncComponents (to, from, next) {
  ...
  try {
    ...
    this.$coinpool.$emit('routeChanged', to, from, err)
    next()
  }
}
...
async function render (to, from, next) {
  ...
  try {
    ...
    if (error.message === 'ERR_REDIRECT') {
      return this.$coinpool.$emit('routeChanged', to, from, error)
    }
    _lastPaths = []
    ...
    this.error(error)
    this.$coinpool.$emit('routeChanged', to, from, error)
    next()
  }
}
...
function fixPrepatch (to, ___) {
  ...
  Vue.nextTick(() => {
    ...
    if (triggerScroll) {
      ...
      window.$coinpool.$nextTick(() => {
        window.$coinpool.$emit('triggerScroll')
      })
    }
...
function nuxtReady (_app) {
  window.onCoinpoolReadyCbs.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window._onCoinpoolLoaded === 'function') {
    window._onCoinpoolLoaded(_app)
  }
  // Add router hooks
  router.afterEach((to, from) => {
    // Wait for fixPrepatch + $data updates
    Vue.nextTick(() => _app.$coinpool.$emit('routeChanged', to, from))
  })
}
...
function hotReloadAPI(_app) {
  ...
  let $components = getNuxtChildComponents(_app.$coinpool, [])
